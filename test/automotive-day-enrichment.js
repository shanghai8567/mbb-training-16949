 ...
 */
const PROCESS_FLOW = `离线(SMT前) → SMT(印刷/SPI/贴片/回流/AOI) → X-Ray(SL全检/MP抽检) → 手插/波峰 → ICT → FCT → 装配/EOL`;

const PRODUCTS = {
  ic: "组合仪表 IC — MCU 预烧、扭矩/ICT、FCT-IC-01",
  hu: "中控 HU — eMMC+主控、DDR BGA、FCT-HU-02",
  dcuC: "座舱域控 DCU-C — MCU+Switch+eMMC、关键 BGA、FCT-DCU-C-03",
  dcuB: "车身域控 DCU-B — MCU 离线、继电器/接插件手插、FCT-DCU-B-04 独立台、CAN/LIN",
  dcuA: "ADAS 域控 DCU-A — SOC+eMMC 离线、大 BGA(U12)、FCT-DCU-A-05、Eth/CAN",
};
...
const DAY_ENRICHMENT = {
  1: {
    ...
    deepDive: {
      title: "五类产品 × 四段工艺（贵司）",
      bullets: [
        "离线@SMT前：ADAS 烧 SOC+eMMC 镜像；车身烧 MCU+CAN 矩阵 Boot",
    ...
    exercises: [
      "在 process-routing 表标出：贵司三条现场规则（SMT前离线 / X-Ray SL·MP / 车身独立 FCT）",
      "用 SIPOC 画 ADAS 域控：供应商料 → 离线 → SMT → FCT → OEM",
    ...
    mcq: [
      ...
      },
      {
        ...
        options: [
          ...
          { v: "c", t: "只审 FCT 不加查离线/SMT", correct: false },
      ...
      },
      {
        ...
        options: [
          { v: "a", t: "否；须 CTQ 对齐、MSA 合格，且 FCT 用 p 图/缺陷矩阵链到离线/BGA", correct: true },
    ...
    keywords: ["工艺段", "离线", "smt", "bga", "fct", "dcu", "域控", "sipoc", "routing"],
  },
  2: {
    focus: "Define — ADAS SCAR Charter",
    conceptAdd:
      "问题陈述示例：「ADAS 域控 FCT Eth 一次通过率由 99.2% 降至 97.1%（n=连续 20 批）」— 禁止写「加强烧录管理」当问题。范围必须写明：离线/SMT/手插/装配/FCT，排除整车路测。",
    deepDive: {
      title: "ADAS SCAR — CTQ 按工艺段分解",
      bullets: [
        "VOC→CTQ：Eth 丢包率、启动时延、功能安全相关项（界定 ASIL 边界）",
        "离线 CTQ：SOC_FW + eMMC_IMG Checksum 与 CSR 一致",
  ...
  3: {
    ...
    conceptAdd:
      "FCT 一次通过率、离线错版率是属性数据 → p/np 图；SPI 体积、BGA 空洞%、扭矩是计量数据 → I-MR/Xbar-R/Cpk。数据计划按工艺段分表，字段须能追溯到 Profile#、烧录批次、X-Ray 阶段(SL/MP)。",
    deepDive: {
      title: "数据字典（按段）",
      bullets: [
        "离线：Program_ID、Checksum_OK、烧录器#、批次",
        "SMT：SPI_vol%、炉温 Profile#、AOI_void%、PCB_ID",
        "手插：不良代码（浮高/通断）、波峰#2（车身）",
        "FCT：Fail_code、工站号、SOC_FW（须与离线一致）",
      ],
    },
    exercises: [
      "为 SPI 体积写操作定义（钢网 ID、测点、单位 %）",
      "说明 FCT 良率分层：DCU-A / DCU-B 分表",
      "选图：离线错版率用 p 还是 Xbar-R",
    ],
    mcq: [
      {
        q: "ADAS 域控「离线 SOC 错版率」宜用哪种图？",
        options: [
  ...
  5: {
    ...
    conceptAdd:
      "用缺陷-过程矩阵：行=Eth_FAIL/CAN_FAIL/Checksum_FAIL；列=离线/SMT/手插/装配。卡方或对比检验比较「错版批次 vs 正常批次」FCT；须对比 X-Ray SL 全检期与 MP 抽检期的空洞 Pareto。",
    deepDive: {
      title: "ADAS SCAR 假设检验路径",
      bullets: [
        "H0：离线版本与 FCT 失败独立 — 用列联表/卡方",
    ...
    mcq: [
      ...
      {
        q: "离线 SOC v2.4.0/v2.4.1 混杂，适宜分析？",
        options: [
    ...
    mcq: [
      {
        ...
        options: [
          { v: "a", t: "分段 MSA 合格 + 离线/SMT/FCT 基线", correct: true },
  ...
  8: {
    ...
    conceptAdd:
      "PFMEA 按离线/SMT/手插/装配/FCT 分行：离线错版 S=10；ADAS U12 空洞 S=9；车身手插浮高 S=8；探测度须对应 X-Ray、Checksum 门禁。",
    deepDive: {
      title: "PFMEA 工艺段行（示例）",
      bullets: [
        "离线@SMT前：SOC 镜像错版 — 探测度 2（Checksum）",
        "SMT：U12 空洞 — 关联 Profile#、AOI、AXI",
        "手插：DCU-B 继电器浮高 — 通断检",
        "FCT：禁止用重测掩盖 SMT/离线问题",
      ],
    },
    exercises: ["pfmea 填 ADAS 离线+SMT 各 1 行", "车身手插浮高 1 行 RPN 排序"],
    homeworkHint: "pfmea-electronics：ADAS+车身各≥2 行；含 S/O/D 与对策",
    mcq: [
      {
        q: "离线错版在 PFMEA 中严重度 S 典型？",
        options: [
  ...
  9: {
    focus: "RSM — SPI/空洞稳健区",
    conceptAdd:
      "确认运行：在 Profile#ADAS-04 稳健区内试产 3 批，每批离线 0 错版、SL 期 X-Ray 全检、FCT≥99.5%。",
    deepDive: {
    ...
    mcq: [
      ...
      },
      {
        ...
        options: [
          { v: "a", t: "离线 0 错版 + FCT≥目标 连续 3 批 + X-Ray SL 全检", correct: true },
  ...
  10: {
    ...
    deepDive: {
      title: "ADAS 控制计划关键行",
      bullets: [
        "050 离线@SMT前 — Checksum 100%",
        "145 X-Ray — SL 全检 / MP 抽检切换条件",
        "420 FCT-05 — p 图 + Fail_code",
      ],
    },
    exercises: ["control-plan 标 ADAS X-Ray 切换条件", "车身 DCU-B 手插+ FCT-04 各 1 行"],
    homeworkHint: "五产品各≥1 工序；ADAS 含离线/SPI/X-Ray/FCT",
    mcq: [
      {
        q: "ADAS 控制计划须单独一行？",
        options: [
          { v: "a", t: "离线 100% Checksum、SPI SPC、X-Ray SL/MP、FCT p 图、装配扭矩", correct: true },
  ...
  11: {
    ...
    deepDive: {
      title: "DMADV × 四段工艺",
      bullets: ["Define：OEM VOC+ASIL 边界", "Measure：NPI Pilot SPI/BGA/FCT MSA", "Verify：不可跳过离线镜像 CTQ"],
    },
    ...
    mcq: [
      {
        ...
        options: [
          { v: "a", t: "新 SOC BGA 设计+离线镜像+新 FCT 场景", correct: true },
  ...
  12: {
    ...
    deepDive: {
      ...
      bullets: ["模板 A：离线错版 p 图", "模板 B：SPI+空洞", "模板 C：车身手插", "模板 D：ADAS FCT Eth"],
    },
  ...
  13: {
    focus: "MBB 辅导",
    conceptAdd:
      "辅导场景：BB 用 FCT-05 数据讲车身问题 — MBB 要求换 FCT-04 序列；BB 只加 FCT 重测 — MBB 要求补离线/SMT 证据。",
    deepDive: {
      title: "MBB 辅导红线",
  ...
  14: {
    focus: "Capstone 答辩",
    conceptAdd:
      "答辩 15min 结构：问题与效益 → 四段证据（离线/SMT/X-Ray/FCT）→ 对车身/座舱无回归 → ASIL 边界声明 → 复制计划。",
    deepDive: {
    ...
    mcq: [
      {
        q: "ADAS SCAR 答辩须展示？",
        options: [
          { v: "a", t: "离线/SMT/X-Ray/FCT 四段证据链与效益", correct: true },
...
function getKeywordsForDay(dayN) {
  ...
  return [...new Set([...base, "离线", "smt", "bga", "fct", "域控", "装配", "手插"])];
}
